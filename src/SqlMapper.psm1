...
class SqlMapper {
	...
	# /// <returns>The value of the given type corresponding to the specified object.</returns>
	# hidden static object? ChangeType(object? value, Type conversionType, bool isNullable = false) {
	...
	# hidden static List<Dictionary<string, object?>> SplitOn(IDataRecord record, params string[] fields) {
	# 	$properties = new List<KeyValuePair<string, object?>>(record.FieldCount);
	# 	for ($index = 0; index < record.FieldCount; index++) {
	# 		$value = record[index];
	...
	.OUTPUTS
		A list of dictionaries representing the objects extracted from the data record.
	#>
